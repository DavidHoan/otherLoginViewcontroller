...
- (void) initWellcomeView
{
    ...
    welcometext.text = @"Welcome to member register!";
...
- (void) initLoginview
{
    NSArray* inputholder = @[@"Tên đăng nhập", @"Mật khẩu", @"Nhập lại mật khẩu", @"Email"];
    
    UIView *loginview = [[UIView alloc] initWithFrame:CGRectMake(0, 58*scalef+100, screenW, 360)];
    
    [self.view addSubview: loginview];
    int posy = 0;
    CGSize itemFrame = CGSizeMake(208, 38);
    for (int i=0; i<inputholder.count; i++) {
        posy = i*55;
        [self initInputField:loginview top: posy size:CGSizeMake(198, 39) holder:inputholder[i]  tag:i fontSize:16.0f];
    }
    
    [self initLoginbtn:loginview top:posy+80 size:itemFrame titleStr:@"ĐĂNG KÝ" fontSize:14.0f mybtn:loginbtn bgColor:RGBColor(0x4f9cdf) borderColor:RGBColor(0x4f9cdf) function:@selector(loginfunction)];
    
    
    [self initLoginbtn:loginview top:posy+ 80 +50 size:itemFrame titleStr:@"User Agreement and Privacy policy" fontSize:14.0f mybtn:loginbtn bgColor:[UIColor clearColor] borderColor:[UIColor clearColor] function:@selector(loginfunction)];
}
...
- (void) initInputField:(UIView*) mainview top:(int)posY size:(CGSize)inputSize holder:(NSString*)holderStr tag:(int)tag fontSize:(float)fontSize
{
    ...
    if (tag==1||tag==2) { inputTextField.secureTextEntry = YES;  }
